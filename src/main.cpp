...
void initOLED();
void initHOURGLASS();
void checkOrientation(int16_t ax, int16_t ay, int16_t az);
...
void updateOLED() {
  ...
  int sandSource = (orientation == 0)   ? countSand(topGrid)
                   : (orientation == 1) ? countSand(botGrid)
                                        : 0;
...
void drawGrid(int addr, byte grid[8][8]) {
  for (int x = 0; x < 8; x++)
    for (int y = 0; y < 8; y++)
      setLedRotated(addr, x, y, grid[x][y]);
}
...
void updatePhysicsSideways(byte grid[8][8]) {
  for (int y = 7; y >= 0; y--)
    for (int x = 7; x >= 0; x--) {
      if (grid[x][y] == 1) {
        // Primary: fall straight down
        if (y < 7 && grid[x][y + 1] == 0) {
          grid[x][y] = 0;
          grid[x][y + 1] = 1;
        }
        // Secondary: diagonal down-right (gravity direction +x)
        else if (y < 7 && x < 7 && grid[x + 1][y + 1] == 0) {
          grid[x][y] = 0;
          grid[x + 1][y + 1] = 1;
        }
        // Tertiary: diagonal down-left (opposite direction)
        else if (y < 7 && x > 0 && grid[x - 1][y + 1] == 0) {
          grid[x][y] = 0;
          grid[x - 1][y + 1] = 1;
        }
        // Quaternary: move right (spread towards gravity)
        else if (x < 7 && grid[x + 1][y] == 0) {
          grid[x][y] = 0;
          grid[x + 1][y] = 1;
        }
        // Quinary: move left (spread away from gravity)
        else if (x > 0 && grid[x - 1][y] == 0) {
          grid[x][y] = 0;
          grid[x - 1][y] = 1;
        }
...
void handleUpdate(unsigned long currentMillis) {
  if (currentMillis - lastPhysics > 40) {
    if (orientation == 0) {
      updatePhysicsDownLeft(topGrid);
      updatePhysicsDownLeft(botGrid);
    } else if (orientation == 1) {
      updatePhysicsUpRight(topGrid);
      updatePhysicsUpRight(botGrid);
    } else if (orientation == 2) {
      updatePhysicsSideways(topGrid);
      updatePhysicsSideways(botGrid);
    } else if (orientation == 3) {
      updatePhysicsSidewaysNegative(topGrid);
      updatePhysicsSidewaysNegative(botGrid);
    }
